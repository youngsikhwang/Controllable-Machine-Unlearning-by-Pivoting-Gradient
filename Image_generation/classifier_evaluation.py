...
def validate(model, loader, args):
    ...
    accuracy_cum_sum = 0
    ood_classification_count = 0  # Count of samples classified as "other class"
    model.eval()
    ...
    for data in tqdm(iter(loader)):
        ...
        # "Other class" classification: assign class 11 if max probability is below threshold
        #max_probs, pred = torch.max(probs, dim=-1)
        print(probs)
        pred = torch.max(probs, dim=-1)
        #pred[max_probs < threshold] = 11  # 11 represents the "other class"
        ...
        # Record the number of samples classified as "other class"
        #ood_classification_count += (pred == 11).sum().item()
    ...
    #print(f"Number of 'other class' classifications: {ood_classification_count}")
    ...
    result = {
        ...
        "accuracy of forgotten class": float(accuracy_cum_sum),
    }
